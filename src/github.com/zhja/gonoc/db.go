...
func (db *DB) GroupBy(field string) {
	if field == "" {
		CheckCustomError("Field is empty")
	} else {
		var group string
...
func (db *DB) OrderBy(field, value string) {
	if field == "" || value == "" {
		CheckCustomError("Field or value is empty")
	} else {
		var order string
...
func (db *DB) where(operate, in, field, value string) {
	if field == "" || operate == "" || value == "" {
		CheckCustomError("Field or value or operate is empty")
	} else {
		var where string
...
//query
func (db *DB) Query() (rs []map[string]string, err error) {
...
func (db *DB) QueryRows() (rs map[string]string, err error) {
	rows, err := db.DB.Query(db.sqlInfo)
	CheckError(err)
	rs = db.GetOneScan(rows)
	db.cleanSql()
	return
}
...
func (db *DB) Delete(info interface{}) {
	...
	for i := 0; i < v.NumField(); i++ {
		if v.Field(i).Interface() != "" && v.Field(i).Interface() != 0 { 
			deleteWhere = fmt.Sprintf("%s AND %s = '%v'",
				deleteWhere,
				typeOfV.Field(i).Name,
				v.Field(i).Interface(),
			)
	...
	db.sqlInfo = fmt.Sprintf("DELETE FROM `%s` WHERE 1=1%s",
		tableName,
		deleteWhere,
	)
	db.DB.Exec(db.sqlInfo)
...
func (db *DB) GetOneScan(rows *sql.Rows) map[string]string {
	columns, err := rows.Columns()
	CheckError(err)
	values := make([]sql.RawBytes, len(columns))
	scanArgs := make([]interface{}, len(values))
